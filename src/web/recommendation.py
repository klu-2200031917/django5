...
def Myrecommend():
    ...
    def reshapeParams(flattened_XandTheta, mynm, mynu, mynf):
        total_size = int(mynm * mynf + mynu * mynf)
        if flattened_XandTheta.shape[0] != total_size:
            raise ValueError("Invalid size for flattened_XandTheta")

        reX = flattened_XandTheta[:mynm * mynf].reshape((mynm, mynf))
        reTheta = flattened_XandTheta[mynm * mynf:].reshape((mynu, mynf))
        return reX, reTheta
    ...
    result = scipy.optimize.fmin_cg(
        cofiCostFunc, x0=myflat, fprime=cofiGrad, args=(Y, R, mynu, mynm, mynf, mylambda), maxiter=40, disp=True,
        full_output=True
    )
